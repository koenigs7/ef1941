...
import { Nationality, Alliance } from '../units/unit/unit.enums';
...
export class AIService {
    ...
    startAI(alliance: Alliance) {
        this.calculateIFRs(alliance);
        this.createReinforcementMoves();
        this.createInActionMoves();
    }

    calculateIFRs(alliance: Alliance) {
        this.reinforcements = [];
        this.inaction = [];
        this.unitService.activeAllies().forEach(
            unit => {
                unit.clearOrders();
                const ifr = new IFR();
                const unitLoc = unit.getLocation();
                this.unitService. activeAxis().forEach(enemyUnit => {
    ...
    createReinforcementMoves() {
        this.reinforcements.forEach(unit => {
            let mostTrouble = 0;
            let unitToSave;
            this.unitService.activeAllies().forEach(testunit => {
